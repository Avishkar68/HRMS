...
const EmployeeTimesheets = () => {
  ...
  const [modalLog, setModalLog] = useState(null); // null = Create, otherwise log object
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      if (modalLog) {
        ...
        // Create timesheet
        const res = await api.post("/timesheets", modalForm);
        setLogs([res.data, ...logs]);
      }
      handleCloseModal();
    } catch (err) {
      setErrorMsg(err.response?.data?.message || "Failed to log timesheet. Verify inputs.");
    } finally {
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto pb-12">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-3xl font-extrabold text-gray-900 tracking-tight">Work Timesheets</h1>
          <p className="text-gray-500 text-sm mt-1">Log your hours and track approval milestones</p>
        </div>
        <button 
      ...
      <div className="bg-white rounded-3xl border border-gray-200 shadow-sm overflow-hidden">
        {filteredLogs.length === 0 ? (
          <div className="p-16 text-center text-gray-400 flex flex-col items-center justify-center gap-3">
            ...
            <h3 className="font-extrabold text-gray-900 text-sm">No work hours logged</h3>
            <p className="text-xs text-gray-400">Timesheets logged will appear here once provisioned.</p>
          </div>
        ) : (
            ...
            <table className="w-full text-sm">
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
                  ...
                  <th className="text-left p-4 font-semibold text-gray-600">Hours Logged</th>
                  <th className="text-left p-4 font-semibold text-gray-600">Description</th>
                  <th className="text-left p-4 font-semibold text-gray-600">Approval Status</th>
                ...
                {filteredLogs.map((log) => {
                  ...
                  return (
                    <tr key={log._id} className="hover:bg-slate-50/50 transition-colors">
                      <td className="p-4 font-medium text-gray-900 font-mono">
                      ...
                      <td className="p-4">
                        <div className="flex flex-col gap-0.5">
                          ...
                          {isRejected && log.rejectionReason && (
                            <span className="text-[10px] text-rose-500 font-bold block max-w-[200px] truncate" title={log.rejectionReason}>
                          ...
                          {isPending ? (
                            <>
                              ...
                              <button
                                ...
                                className="p-1.5 text-gray-400 hover:text-rose-650 hover:bg-rose-50/50 border border-transparent hover:border-rose-100 rounded-xl transition-all cursor-pointer"
                                title="Delete Log"
                              >
                            ...
                            <span className="text-[10px] text-gray-400 font-bold uppercase tracking-wider italic select-none">Locked</span>
                          )}
                        </div>
                      </td>
                    </tr>
          ...
          <div className="bg-white rounded-3xl shadow-2xl border border-gray-150 w-full max-w-md p-6 relative z-10 animate-in fade-in zoom-in-95 duration-200">
            <div className="flex items-center justify-between border-b border-gray-100 pb-4 mb-4">
              <h3 className="text-lg font-black text-gray-900">
                {modalLog ? "Edit Timesheet Entry" : "Log Daily Work Hours"}
              </h3>
              <button 
                onClick={handleCloseModal}
            ...
            <form onSubmit={handleFormSubmit} className="space-y-4">
              ...
              <div className="space-y-1">
                ...
                <input 
                  type="text"
                  placeholder="e.g. HRMS Redesign, General Operations"
              ...
              <div className="space-y-1">
                ...
                <textarea 
                  placeholder="Describe your work deliverables or activities..."
                  value={modalForm.description}
                  onChange={(e) => setModalForm({...modalForm, description: e.target.value})}
                  className="w-full border border-gray-250 rounded-xl p-2.5 text-sm focus:ring-2 focus:ring-indigo-500 outline-none min-h-[80px] max-h-[140px]"
                />
              </div>
              ...
              <div className="flex items-center justify-end gap-3 pt-4 border-t border-gray-100 mt-6">
                ...
                >
                  ...
                  <span>{modalLog ? "Save Changes" : "Submit Hours"}</span>
                </button>
              </div>
            </form>
          </div>
