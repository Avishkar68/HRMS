...
const ManagerServiceBook = () => {
  ...
  const fetchMemberServiceBook = async (memberId) => {
    if (!memberId) return;
    setEntriesLoading(true);
    try {
      const res = await api.get(`/service-book?employeeId=${memberId}`);
      setEntries(res.data || []);
    } catch (err) {
      console.error("Error fetching member service book:", err);
    } finally {
  ...
  useEffect(() => {
    if (selectedMemberId) {
      fetchMemberServiceBook(selectedMemberId);
    } else {
      setEntries([]);
    }
  }, [selectedMemberId]);
  ...
  const handleOpenCreateModal = () => {
    setErrorMsg("");
    // Autofill current designation/department if entries exist
  ...
  const handleFormSubmit = async (e) => {
    ...
    if (!modalForm.eventType || !modalForm.eventDate || !modalForm.designation || !modalForm.department || !modalForm.officeOrderNumber) {
    ...
    try {
      ...
      setErrorMsg(err.response?.data?.message || "Failed to log service record entry.");
    } finally {
  ...
  const handleDeleteEntry = async (id) => {
    if (!window.confirm("Are you sure you want to delete this service book entry? This action is permanent.")) return;
  ...
  const currentMember = team.find(m => m._id === selectedMemberId);
  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto pb-12">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-3xl font-extrabold text-gray-900 tracking-tight">Team Service Records</h1>
          <p className="text-gray-500 text-sm mt-1">Audit and record professional career milestones for direct reportees</p>
        </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-4 flex flex-col sm:flex-row items-center gap-4">
        <label className="text-xs font-bold text-gray-400 uppercase tracking-wider whitespace-nowrap">Choose Team Member:</label>
        <select
      ...
      {selectedMemberId ? (
        <div className="bg-white rounded-3xl border border-gray-200 shadow-sm p-6 relative">
          <div className="border-b border-gray-100 pb-4 mb-6 flex items-center justify-between">
            <div>
              <h3 className="font-extrabold text-gray-900 text-base">Timeline Directory</h3>
              <p className="text-xs text-gray-400">Chronological list of career events for {currentMember?.name}</p>
            </div>
        ...
        <div className="bg-white rounded-3xl border border-gray-200 shadow-sm p-16 text-center text-gray-450 flex flex-col items-center justify-center gap-3">
          ...
          <h3 className="font-extrabold text-gray-900 text-sm">Select a team member to view history</h3>
          <p className="text-xs text-gray-400">Choose one of your active direct reporting employees to examine or append their corporate service records.</p>
        </div>
      )}

      {/* Log Service Book Entry Modal */}
      {showCreateModal && (
...
export default ManagerServiceBook;
